...
public class DiceApplication extends Application {
    ...
    public boolean can_continue = true;
    @FXML
    CheckBox assist;
    ...
    public void scene_manager(int scene) throws IOException {
        ...
        switch (scene) {
            case 1:
                ...
                break;
            case 2:
                against_bot = gamemode1.isSelected();
                if (against_bot) {
                    player_count = 2;
                } else {
                ...
                if (!against_bot) {
                    for (int i = 0; i < player_count; i++) {
                        ...
                        textAreas[i].setStyle("-fx-background-color:#b5b5b5;");
                ...
                text_holder.setStyle("-fx-background-color:grey");
                ScrollPane tmp_scroll = new ScrollPane(text_holder);
                tmp_scroll.setStyle("-fx-background-color:grey");
                tmp_scroll.setFitToWidth(true);
                tmp_scroll.setFitToHeight(true);
                Scene scene6 = new Scene(tmp_scroll);
            ...
            case 4:
                ...
                end_turn.setStyle("-fx-background-color:#b5b5b5");
                ...
                naseptavac.setStyle("-fx-background-radius:0 0 10 10;-fx-background-color:grey");
    ...
    public void generate_values(CuboidMesh[] ar_box, RotateTransition[] transitionRX, TranslateTransition[] transitionT, boolean assist) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < 6; i++) {
            if (locked_dice[i] == 0) {
                ...
                switch (random1) {
                    case 1:
                        calculate_cube_rotation(transitionRX[i], Math.toRadians(0), Math.toRadians(180), Math.toRadians(270));
                        break;
                    case 2:
                        calculate_cube_rotation(transitionRX[i], Math.toRadians(0), Math.toRadians(90), Math.toRadians(0));
                        break;
                    case 3:
                        calculate_cube_rotation(transitionRX[i], Math.toRadians(0), Math.toRadians(0), Math.toRadians(180));
                        break;
                    case 4:
                        ...
                        break;
                    case 5:
                        calculate_cube_rotation(transitionRX[i], Math.toRadians(0), Math.toRadians(270), Math.toRadians(0));
                        break;
                    case 6:
                        calculate_cube_rotation(transitionRX[i], Math.toRadians(0), Math.toRadians(180), Math.toRadians(90));
                        break;
                }
            }
        }
    ...
    private void evaluate_throw(int[] dice_values,int mode) {
        ...
        if (mode == 3){
            System.out.println(Arrays.toString(dice_values));
